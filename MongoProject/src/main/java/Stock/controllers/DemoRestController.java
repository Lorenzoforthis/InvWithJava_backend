...
public class DemoRestController {
    ...
    @GetMapping("/{id}")
    public DemoModel getById(@PathVariable(name="id")String id){
        return service.getById(id);
    }

    //parameters comes from Thymeleaf form
    @PostMapping("/")
    public String addData(@RequestBody DemoModel model) {
        return service.addOne(model);    }

  
    @PutMapping("/update/{id}")
    public DemoModel update(@RequestBody DemoModel model,@PathVariable(name="id") String  id){
