...
public class FinancialDataRepository {
    ...
    public void batchInsertFinancialData(List<FinancialDataDTO> dataList) {
        try {
            ...
            for (int i = 0; i < dataList.size(); i += BATCH_SIZE) {
                ...
                List<FinancialDataDTO> batch = dataList.subList(i, endIndex);
    ...
    private void processBatch(List<FinancialDataDTO> batch) {
        jdbcTemplate.batchUpdate(INSERT_SQL, new BatchPreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                FinancialDataDTO data = batch.get(i);
